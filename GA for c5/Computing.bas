...
Dim Work(), Finish, Critical(), News As Boolean
Dim G() As Integer
Dim VGo(), tV(), v, Boundry(), f, w As Single
Dim p, l, Depth, n, Suggest, Must, k, WP, Near(60) As Integer
Dim Force(0 To 30), Delay, GoUp, ex(0 To 64) As Boolean
Public ValueX As Single
Private Sub Choose()
Static x, Temp As Integer
Static vv, v As Single
Static fc As Boolean
If News = True Then
News = False
If Critical(n - 1) = False Then
    If OnEarth(Critical(n), c3 - n) = True Then
        VGo(n) = 1000 - n - WP
         n = n - 1
       Work(G(n), n) = True
        Turn = -Turn
...
End Function
Private Sub Base()
Dim wait(64), mm, Choice As Integer
Dim x, vv, z, VChoice(64) As Single
Finish = False
Critical(1) = False
n = 1
Turn = ToGo
...
For x = 1 To 64
 ...
 End If
Next
If mm < 1 Then MsgBox (mm & "," & VGo(1) & "," & VGo(2))
  Randomize
  mm = Rnd() * mm + 0.5
  Suggest = wait(mm)
  ValueX = VGo(1)
